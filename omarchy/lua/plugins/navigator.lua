return {
  "ray-x/navigator.lua",
  init = function()
    -- Shim: navigator calls nvim_buf_get_option_value which doesn't exist in 0.10+
    -- (API is nvim_get_option_value(name, { buf = bufnr }))
    if vim.api.nvim_buf_get_option_value == nil then
      vim.api.nvim_buf_get_option_value = function(name, opts)
        return vim.api.nvim_get_option_value(name, opts or {})
      end
    end
  end,
  dependencies = {
